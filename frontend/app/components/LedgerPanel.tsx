    ...
    <div className="card formcard">
      ...
      {days.length === 0 ? (
        <div className="emptynote">
          Nothing logged for this month yet. Kakeibo is written by hand for a reason — the act of
          entering each spend is what makes it conscious.
        </div>
      ) : (
              ...
              <div className="daygroup" key={day}>
                ...
                {rows.map((entry) => (
                  <div className="entry" key={entry.id}>
                    <i className="key" style={{ background: CATEGORY_COLOR[entry.category] }} />
                    <span className="note">
                    ...
                    <DeleteButton id={entry.id} note={entry.note} />
                  </div>
                ))}
              </div>
            );
...
function DeleteButton({ id, note }: { id: number; note: string }) {
  ...
  return (
    <fetcher.Form method="post">
      ...
      <button
        ...
        type="submit"
        title="Delete entry"
        aria-label={`Delete entry${note ? ` “${note}”` : ""}`}
